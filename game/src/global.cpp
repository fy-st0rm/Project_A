...
std::shared_ptr<Sparky::OrthoCamera>   Global::ui_camera = nullptr;
...
void Global::init()
{
	...
	Global::ui_camera       = std::make_shared<Sparky::OrthoCamera>(glm::vec3(0,0,0), 0.0f, ZOOM_WIDTH, 0.0f, ZOOM_HEIGHT, -100.0f, 100.0f);
	Global::renderer     = std::make_shared<Sparky::QuadRenderer>(MAX_QUAD_CNT, Global::camera);
	Global::ui_renderer  = std::make_shared<Sparky::UIRenderer>(MAX_QUAD_CNT, Global::ui_camera);
